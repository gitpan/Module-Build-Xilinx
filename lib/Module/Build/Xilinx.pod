...
simulate and program a Xilinx FPGA development board using Xilinx ISE Webpack.
...
it to add the VHDL files that are appropriate for the build that we would like to do.
...
but also find the VHDL sources, creates the Tcl script and finds the Xilinx ISE
...
This action creates the Xilinx ISE project file and adds the other VHDL sources
...
This action cleans the Xilinx ISE project file and removes all the existing VHDL
...
This action runs the ISimGui from Xilinx to view the output of the simulation
run through the C<test> action above. For each test bench, the ISimGui will be
...
commandline option. This commandline option is necessary for the programming to
happen.
The recommended way to run it is:
...
The language supported in the project which as of version 0.03 is VHDL. We do
...
different testbench executables need to be creating mapping the correct VHDL
...
where C<$filename> is the name of the testbench VHDL source file without the
...
I<$filename.exe> where C<$filename> is the name of the testbench VHDL source file without the
extension. Unless you really want to, this is automatically set by
L<Module::Build::Xilinx> and it is better to leave it that way.
...
Default is I<$filename.cmd> where C<$filename> is the name of the testbench VHDL source file without the
extension. Unless you really want to, this is automatically set by
L<Module::Build::Xilinx> and it is better to leave it that way.
...
software. Default is I<$filename.wdb> where C<$filename> is the name of the testbench VHDL source file without the
extension. Unless you really want to, this is automatically set by
L<Module::Build::Xilinx> and it is better to leave it that way.
